...
class StrategyObservation:
    def __init__(self, timepoint,
        ...
        self.tickSpacing = int(self.fee_tier * 2 * 10000) if self.fee_tier > (100 / 1e6) else int(
            self.fee_tier * 10000)  # 1bp pool's tick spacing is 1x the fee tier, other pool's 2x
            ...
            for s in range(len(relevant_swaps)):
                for i in range(len(self.liquidity_ranges)):
                    ...
                    if relevant_swaps.iloc[s]['virtual_liquidity'] < 1e-9:
                        ...
                        fraction_fees_earned_position = self.liquidity_ranges[i]['position_liquidity'] / (
                                self.liquidity_ranges[i]['position_liquidity'] + relevant_swaps.iloc[s][
                                'virtual_liquidity'])

                    fees_earned_token_0 += in_range * token_0_in * self.fee_tier * fraction_fees_earned_position * \
                                           relevant_swaps.iloc[s]['traded_in']
...
def generate_simulation_series(simulations, strategy_in, token_0_usd_data=None):
    ...
    if token_0_usd_data is None:
        ...
        data_strategy['cum_fees_usd'] = data_strategy['token_0_fees'].cumsum() + (
                data_strategy['token_1_fees'] / data_strategy['price']).cumsum()
        ...
        data_return = pd.merge_asof(data_strategy, token_0_usd_data['price_0_usd'], on='time_pd', direction='backward',
        ...
        data_return['base_position_value_usd'] = data_return['base_position_value_in_token_0'] * data_return[
            'price_0_usd']
        data_return['limit_position_value_usd'] = data_return['limit_position_value_in_token_0'] * data_return[
            'price_0_usd']
        data_return['cum_fees_0'] = data_return['token_0_fees'].cumsum() + (
                data_return['token_1_fees'] / data_return['price']).cumsum()
...
def aggregate_price_data(data, frequency):
    ...
    price_range = pd.DataFrame(
        {'time_pd': pd.date_range(data_floored_min.index.min(), data_floored_min.index.max(), freq='1 min', tz='UTC')})
    price_range = price_range.set_index('time_pd')
...
def analyze_strategy(data_usd, frequency='M'):
    ...
    summary_strat = {
        ...
        'max_drawdown': (data_usd['value_position_usd'].max() - data_usd['value_position_usd'].min()) / data_usd[
            'value_position_usd'].max(),
        'volatility': ((data_usd['value_position_usd'].pct_change().var()) ** (0.5)) * (
                (annualization_factor) ** (0.5)),
        'sharpe_ratio': float(net_apr / (
                ((data_usd['value_position_usd'].pct_change().var()) ** (0.5)) * ((annualization_factor) ** (0.5)))),
        'impermanent_loss': (
                (strategy_last_obs['value_position_usd'] - strategy_last_obs['value_hold_usd']) / strategy_last_obs[
                'value_hold_usd'])[0],
        'mean_base_position': (data_usd['base_position_value_in_token_0'] / \
                               (data_usd['base_position_value_in_token_0'] + data_usd[
                                   'limit_position_value_in_token_0'] + data_usd['value_left_over_in_token_0'])).mean(),
        'median_base_position': (data_usd['base_position_value_in_token_0'] / \
                                 (data_usd['base_position_value_in_token_0'] + data_usd[
                                     'limit_position_value_in_token_0'] + data_usd[
                                      'value_left_over_in_token_0'])).median(),
        'mean_base_width': (
                (data_usd['base_range_upper'] - data_usd['base_range_lower']) / data_usd['price_at_reset']).mean(),
        'median_base_width': (
                (data_usd['base_range_upper'] - data_usd['base_range_lower']) / data_usd['price_at_reset']).median(),
