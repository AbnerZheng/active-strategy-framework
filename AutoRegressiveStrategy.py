...
class AutoRegressiveStrategy:
    def __init__(self, model_data, alpha_param, tau_param, volatility_reset_ratio, tokens_outside_reset=.05,
                 data_frequency='D', default_width=.5, days_ar_model=180, return_forecast_cutoff=0.15,
                 z_score_cutoff=5):
    ...
    def generate_model_forecast(self, timepoint):
        ...
        current_data = self.model_data.loc[:timepoint].resample(self.resample_option, closed='right', label='right',
    ...
    def check_compound_possible(self, current_strat_obs):
        ...
        base_assets_token_1 = current_strat_obs.liquidity_ranges[0]['token_0'] * current_strat_obs.price + \
                              current_strat_obs.liquidity_ranges[0]['token_1']
        limit_assets_token_1 = current_strat_obs.liquidity_ranges[0]['token_0'] * current_strat_obs.price + \
                               current_strat_obs.liquidity_ranges[0]['token_1']
        ...
        liquidity_placed_base = int(
            UNI_v3_funcs.get_liquidity(current_strat_obs.price_tick_current, baseLower, baseUpper, unused_token_0, \
        ...
        liquidity_placed_limit = int(
            UNI_v3_funcs.get_liquidity(current_strat_obs.price_tick_current, limitLower, limitUpper, \
                                       limit_amount_0, limit_amount_1, current_strat_obs.decimals_0,
                                       current_strat_obs.decimals_1))
        ...
        if ((base_assets_after_compound_token_1 + limit_assets_after_compound_token_1) / (
            base_assets_token_1 + limit_assets_token_1) - 1) > .01:
            return True
        else:
            return False
    ...
    def check_strategy(self, current_strat_obs):
        ...
        model_forecast = None
        LIMIT_ORDER_BALANCE = current_strat_obs.liquidity_ranges[1]['token_0'] * current_strat_obs.price + \
                              current_strat_obs.liquidity_ranges[1]['token_1']
        BASE_ORDER_BALANCE = current_strat_obs.liquidity_ranges[0]['token_0'] * current_strat_obs.price + \
                             current_strat_obs.liquidity_ranges[0]['token_1']
        ...
        left_over_balance = (
                                    current_strat_obs.token_0_left_over + current_strat_obs.token_0_fees_uncollected) * current_strat_obs.price \
    ...
    def set_liquidity_ranges(self, current_strat_obs, model_forecast=None):
        ...
        liquidity_placed_base = int(
            UNI_v3_funcs.get_liquidity(current_strat_obs.price_tick_current, baseLower, baseUpper,
        ...
        current_strat_obs.token_0_left_over = max(
            [total_token_0_amount - base_amount_0_placed - limit_amount_0_placed, 0.0])
        current_strat_obs.token_1_left_over = max(
            [total_token_1_amount - base_amount_1_placed - limit_amount_1_placed, 0.0])
    ...
    def dict_components(self, strategy_observation):
        ...
        this_data[
            'token_0_total'] = total_token_0 + strategy_observation.token_0_left_over + strategy_observation.token_0_fees_uncollected
        this_data[
            'token_1_total'] = total_token_1 + strategy_observation.token_1_left_over + strategy_observation.token_1_fees_uncollected
