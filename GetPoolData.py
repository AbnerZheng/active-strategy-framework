...
def download_bigquery_swap_data(contract_address, date_begin, date_end, network='ethereum', block_start=0,
    ...
    query = """
            ...
            where contract_address = lower('""" + contract_address.lower() + """') and
              block_timestamp >= '""" + str(date_begin) + """' and block_timestamp <= '""" + str(
        date_end) + """' and block_number >= """ + str(block_start) + """
            """
...
def get_pool_data_bigquery(contract_address, date_begin, date_end, decimals_0, decimals_1, network='mainnet',
                           block_start=0, credentials=None):
    """
    ...
    if network == 'mainnet':
        resulting_data = download_bigquery_swap_data(contract_address.lower(), date_begin, date_end, network='ethereum',
                                                     block_start=block_start, credentials=credentials)
    elif network == 'polygon':
        resulting_data = download_bigquery_swap_data(contract_address.lower(), date_begin, date_end, network=network,
                                                     block_start=block_start, credentials=credentials)
    else:
    ...
    resulting_data['virtual_liquidity_adj'] = resulting_data['liquidity'].astype(float) / (
        10 ** ((decimals_0 + decimals_1) / 2))
    resulting_data['token_in'] = resulting_data.apply(lambda x: 'token0' if (x['amount0_adj'] < 0) else 'token1',
                                                      axis=1)
    resulting_data['traded_in'] = resulting_data.apply(
        lambda x: -x['amount0_adj'] if (x['amount0_adj'] < 0) else -x['amount1_adj'], axis=1).astype(float)
...
def get_swap_data(contract_address, file_name, DOWNLOAD_DATA=True, network='mainnet'):
    """
    Internal function to query full history of swap data from Uniswap v3's subgraph. Use
    GetPoolData.get_pool_data_flipside which preprocesses the data in order to conduct simulations with the Active
    Strategy Framework.
    """
    ...
    if DOWNLOAD_DATA:
        ...
        while not finished:
            ...
            response = \
                query_univ3_graph(current_payload, variables={'paginateId': current_id}, network=network)['data'][
                    'pool'][
                    'swaps']
...
def get_pool_data_flipside(contract_address, flipside_query, file_name, DOWNLOAD_DATA=True):
    """
    Queries Uniswap v3's subgraph for swap data and Flipside Crypto's queries to find liquidity in order to conduct
    simulations using the Active Strategy Framework.
    """
    ...
    full_data = pd.merge_asof(swap_data, stats_data[['VIRTUAL_LIQUIDITY_ADJUSTED', 'tick_pool']], on='time_pd',
...
def generate_event_payload(event, address, n_query):
    payload = '''query($paginateId: String!){
                          swaps(
                            first: ''' + n_query + '''
                            orderBy: id
                            orderDirection: asc
...
def get_swap_data_univ2(contract_address, file_name, date_begin, date_end, DOWNLOAD_DATA=True, RATE_LIMIT=False):
    ...
    swap_data['amount0'] = swap_data.apply(
        lambda x: -float(x['amount0In']) if x['token_in'] == 'token0' else float(x['amount0Out']), axis=1)
    swap_data['amount1'] = swap_data.apply(
        lambda x: float(x['amount1Out']) if x['token_in'] == 'token0' else -float(x['amount1In']), axis=1)
...
##############################################################
def get_price_data_bitquery(token_0_address, token_1_address, date_begin, date_end, api_token, file_name,
                            DOWNLOAD_DATA=True, RATE_LIMIT=False, exchange_to_query='Uniswap'):
    """
    ...
    if DOWNLOAD_DATA:
        ...
        current_request = run_bitquery_query(
            generate_price_payload(token_0_address, token_1_address, date_begin, date_end, offset, exchange_to_query),
            api_token)
        ...
        while len(current_request['data']['ethereum']['dexTrades']) == max_rows_bitquery:
            current_request = run_bitquery_query(
                generate_price_payload(token_0_address, token_1_address, date_begin, date_end, offset,
                                       exchange_to_query), api_token)
            request.append(current_request)
...
def get_price_usd_data_bitquery(token_address, date_begin, date_end, api_token, file_name, DOWNLOAD_DATA=True,
                                RATE_LIMIT=False, exchange_to_query='Uniswap'):
    """
    ...
    if DOWNLOAD_DATA:
        ...
        offset = 0
        current_request = run_bitquery_query(
            generate_usd_price_payload(token_address, date_begin, date_end, offset, exchange_to_query), api_token)
        request.append(current_request)
